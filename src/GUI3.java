...
class GUI3 extends JPanel {
    ...
    private GraphicsPanel puzzleGraphics;
	 ...
	 public GUI3(int row, int col, int cell_size,String s) {
		 ...
		 score.addActionListener(new ActionListener(){
			 public void actionPerformed(ActionEvent e){
				 ...
				 f.add(new JLabel(puzzleModel.record3[0]));
		   		 f.add(new JLabel(puzzleModel.record3[3],SwingConstants.RIGHT));
		   		 f.add(new JLabel("INTERMEDIATE: "));
		   		 f.add(new JLabel(puzzleModel.record3[1]));
		   		 f.add(new JLabel(puzzleModel.record3[4],SwingConstants.RIGHT));
		   		 f.add(new JLabel("ADVANCED: "));
		   		 f.add(new JLabel(puzzleModel.record3[2]));
		   		 f.add(new JLabel(puzzleModel.record3[5],SwingConstants.RIGHT));
		...
		public GraphicsPanel() {
	     ...
	     public void mousePressed(MouseEvent e) {
	        //--- map x,y coordinates into a row and col.
	         ...
	         if(puzzleModel.isGameOver()){
	        	 if(level=="BEGINNER"){
			    		if(Integer.parseInt(puzzleModel.record3[3])>puzzleModel.count){
			    			String out="NEW RECORD \n you finished the BEGINNER mode in "+puzzleModel.count+" moves"+
				    		"\nnext level is INTERMEDIATE level\n\nPlease enter your name";
			    			puzzleModel.record3[0]=JOptionPane.showInputDialog(null,out);
			    			puzzleModel.record3[3]=""+puzzleModel.count;
			    		}
			    		f.setVisible(false);
			    		new GUI3(6,6,55,"INTERMEDIATE");
			    	}
			    	else if(level=="INTERMEDIATE"){
			    		if(Integer.parseInt(puzzleModel.record3[4])>puzzleModel.count){
			    			String out="NEW RECORD \nyou finished the INTERMEDIATE mode in "+puzzleModel.count+
				    		" moves"+"\nnext level is ADVANCED level\n\nPlease enter your name";
			    			puzzleModel.record3[1]=JOptionPane.showInputDialog(null,out);
			    			puzzleModel.record3[4]=""+puzzleModel.count;
			    			}
			    		f.setVisible(false);
			    		new GUI3(9,9,35,"ADVANCED");
			    	}
			    	else if(level=="ADVANCED"){
			    		if(Integer.parseInt(puzzleModel.record3[5])>puzzleModel.count){
			    			String out="NEW RECORD \nyou finished the ADVANCED mode in "+puzzleModel.count+
				    		" moves"+"\n\nPlease enter your name";
			    			puzzleModel.record3[2]=JOptionPane.showInputDialog(null,out);
			    			puzzleModel.record3[5]=""+puzzleModel.count;
			    			}
			    		f.setVisible(false);
